// Package config loads and saves the spacectl configuration file
// (~/.config/spacectl/config.yaml).
package config
...
const EnvServer = "SPACECTL_SERVER"
...
// Config is the persisted spacectl configuration.
type Config struct {
...
// Dir returns the spacectl configuration directory, honoring
// XDG_CONFIG_HOME and defaulting to ~/.config/spacectl.
...
// ResolveServer applies the precedence flag > SPACECTL_SERVER env > config.
