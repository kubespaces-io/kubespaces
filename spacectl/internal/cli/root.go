// Package cli wires the spacectl cobra commands.
package cli
...
// NewRootCmd builds the spacectl command tree.
func NewRootCmd(build BuildInfo) *cobra.Command {
	root := &cobra.Command{
		Use:           "spacectl",
		Short:         "spacectl controls KubeSpaces tenants",
		Long:          "spacectl is the command-line client for the KubeSpaces API:\nlog in, manage tenants, and grab kubeconfigs for their virtual clusters.",
		SilenceUsage:  true,
		SilenceErrors: true,
	}
